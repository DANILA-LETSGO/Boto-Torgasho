...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Один проход сети по произвольному окну дельт (для проката в       |
//| экзамене). RSI считается по самому окну — как в Predict().        |
//| Остальные доп. входы берутся из кэша базового примера: при        |
...
double ForwardOnWindow(double &win[], int baseSample, double localMin, double range)
  {
   double rsiIn = (CalcRSI(win, p - 1, 14) - 50.0) / 100.0;
   double rsiIn2 = (CalcRSI(win, p - 1, 28) - 50.0) / 100.0;
   ...
   for(int iSample = trainSamples; iSample < trainSamples + validSamples; iSample++)
     {
      ...
      // Тупой прогноз: изменения цены не будет, то есть предсказываем 0.
      ...
      double predDelta = outputMain * range + localMin; // прогноз изменения цены
      ...
      if(examDirBars >= 1 && iSample + p + K <= trainSize)
        {
         double rollWin[p];
         for(int j=0; j<p; j++) rollWin[j] = etalons[iSample + j];
         ...
         for(int step=0; step<Hend; step++)
           {
            ...
            actualSum += etalons[iSample + p + step];
           }
         if(actualSum != 0)
           {
            dirHorTotalBar++;
         ...
         double actualSig = 0;
         for(int step=0; step<Hsig; step++) actualSig += etalons[iSample + p + step];
